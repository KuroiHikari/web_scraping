...
def import_cars() -> None:
    ...
    car_dealer[['Year', 'Brand', 'Typename']] = car_dealer['Name'].apply(extract_info).apply(pd.Series)

    # Drop the original 'Name' column if needed
    car_dealer = car_dealer.drop('Name', axis=1)

    # Reorder columns for clarity if needed
    car_dealer = car_dealer[['Year', 'Brand', 'Typename', 'Price', 'Mileage', 'Rating', 'Reviews', 'Dealer']]
